...
public final class MonthlyBarChartExporter {
    ...
    /**
     * Exports a PNG for the current month (system timezone) to the given Uri.
     *
     * @param context  any context
     * @param resolver content resolver (e.g., Activity#getContentResolver())
     * @param uri      SAF-created destination (MIME: image/png)
     * @throws IOException if output stream cannot be opened or written
     */
    ...
    /**
     * Exports a PNG for a specific year-month to the given Uri.
     *
     * @param context  any context
    ...
    public static void exportMonth(
        ...
        List<Table> rows = DbProvider.get(context).tableDao()
     ...
     *
     * @param context any context
     * @return bitmap ready to show in an ImageView
     * @throws IOException if DB access fails
     */
    public static Bitmap buildCurrentMonthBitmap(Context context)
            throws IOException {
        YearMonth ym = YearMonth.now(ZoneId.systemDefault());
     ...
     * @throws IOException if DB access fails
     */
    public static Bitmap buildMonthBitmap(Context context, int year, int month)
            throws IOException {
        YearMonth ym = YearMonth.of(year, month);
        ZoneId zone = ZoneId.systemDefault();
    ...
    /**
     * Aggregates rows into daily sums for the given month.
     */
    private static double[] buildDaySums(
            List<Table> rows,
            YearMonth ym,
            ZoneId zone
    ) {
        ...
        for (Table row : rows) {
            LocalDate d = Instant.ofEpochMilli(row.timeMillis)
                    .atZone(zone).toLocalDate();
            if (d.getYear() == ym.getYear()
                    && d.getMonthValue() == ym.getMonthValue()) {
                int idx = d.getDayOfMonth() - 1;
                daySum[idx] += row.amountMinor / 100.0; // cents → dollars
            }
        }
        return daySum;
    }
    ...
    /**
     * Draws the bar chart into a new bitmap and returns it.
     */
    private static Bitmap drawChartBitmap(double[] daySum, int year, int month) {
        ...
        canvas.drawText("单位：元", WIDTH - MARGIN_RIGHT - 140, MARGIN_TOP + 8, text);

        // Bars and X labels
        int days = daySum.length;
        float barW = Math.max(8f, (plotW - BAR_GAP * (days + 1)) / days);
        int step = Math.max(1, (int) Math.ceil(days / 15.0));
    ...
    private static void writeToUri(
        ...
        try (OutputStream os = resolver.openOutputStream(uri)) {
            if (os == null) {
                throw new IOException("openOutputStream returned null: " + uri);
            }
